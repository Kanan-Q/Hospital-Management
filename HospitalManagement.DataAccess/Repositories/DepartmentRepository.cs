...
namespace HospitalManagement.DataAccess.Repositories
{
    public class DepartmentRepository(AppDbContext _sql) : IDepartmentRepository
    {
        ...
        public async Task<ICollection<Department>> GetSearchByDepartments(string department) => await _sql.Departments.Where(x => x.DepartmentName == department).ToListAsync();
    }
}
